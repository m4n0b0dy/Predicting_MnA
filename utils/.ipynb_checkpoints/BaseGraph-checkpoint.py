...
class BaseGraph(nx.MultiDiGraph):
	...
	def overwrite_graph(self, new_graph):
		...
		self.add_edges_from(new_graph.edges(keys=True,data=True))
	...
	def pull_from_neo(self, query:str, overwrite:bool = False):
		...
		for node in list(results.graph()._nodes.values()):
			try:
				label = [_ for _ in node._labels if _!= 'Tmp'][0]
			except:
				continue
		...
		for rel in rels:
			_tmp_graph.add_edge(rel.start_node.id,
				rel.end_node.id,
				key=rel.id,
				label=rel.type) #you have to call this label to get stellagraph to recognize it, lmfao
		if overwrite:
